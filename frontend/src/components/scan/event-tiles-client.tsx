...
export function EventTilesClient({ events }: EventTilesClientProps) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      const currentUser = authData.user || authData; // Handle both old and new data structure
      setUser(currentUser);
    }
    
    // The events are already filtered by the parent component, so just use them as-is
            ...
            <Card key={event.id} className="flex flex-col">
              ...
              <CardFooter>
                {(() => {
                  ...
                  const isButtonDisabled = !scanningEnabled || (event.status !== 'ongoing' && event.status !== 'ONGOING');
