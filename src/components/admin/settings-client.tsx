...
import { PlusCircle, LogOut, UserPlus, Shield, Users, Edit2, Trash2 } from 'lucide-react';
...
export function SettingsClient({ users }: SettingsClientProps) {
  ...
  const handleSaveUser = async () => {
    ...
    try {
      const response = await fetch(`/api/users/${editUserModal.user?.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/users/${deleteUserModal.user.id}`, {
        method: 'DELETE',
      });
                ...
                <TableBody>
                  {users.map((user) => (
                    <TableRow key={user.id} className="hover:bg-muted/50">
                      ...
                      <TableCell className="cursor-pointer" onClick={() => handleEditUser(user)}>
                        {user.role === 'admin' ? (
                          <span className="text-sm">{user.pin}@example.com</span>
                        ) : (
